...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
   ...
   //if found it return the value
    ...
    if(first_larger_idx > ((subroot->elements).size()) || first_larger_idx == ((subroot->elements).size())){
      if(subroot->is_leaf){
        return V(); //V()is a default returned
      }
      else{
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    size_t first_larger_idx = insertion_idx(subroot->elements, pair);
        /* TODO Your code goes here! */
    //if we found the element, just return
    ...
    //if we didnt find it and we check if its a leaf node, just insert it
    if(subroot->is_leaf){
        (subroot->elements).insert(subroot->elements.begin()+first_larger_idx,pair);
        return;
    }
    //if not a leaf node and we didnt find it, keep recursively finding
    if(!subroot->is_leaf){
        insert(subroot->children[first_larger_idx],pair);
    }
    //check if the insertion is valid
    //if it cause overflow, split child
    if(((subroot->children)[first_larger_idx]->elements).size() == order || (((subroot->children)[first_larger_idx]->elements).size() > order)){
