...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
     ...
     */
     //if they have different length, they can not be the same string
     if(word.length()!=test.length()){
       return false;
     }
     //otherwise, create a hastable to sort the appearing chars in input
    Dict<char, int> hashTable(256);

    for (size_t i = 0; i < word.length(); i++){
        hashTable[word[i]]++;
        }
    //go through the test string to deducte from the hastable
    for(size_t i = 0; i < test.length(); i++){
      hashTable[test[i]]--;
    }
    //if the hashtable is empty and no other weird thing happen (like all chars in word has been covered and char in test doesnt left alone), return true
    for(size_t i =0; i < word.length(); i++){
      if(hashTable[word[i]] !=0){
        return false;
      }
    }
    for(size_t i =0; i < test.length(); i++){
      if(hashTable[test[i]]!=0){
        return false;
      }
    }
    return true;
