...
//allocator allocated memories in heap but default destructor doesnot free any heap memory
//so it needs customized destuctor to free memory allocated
//since alpha and rooms are the two pointers of arrays
//so we need to use delete[] instead of delete
Allocator::~Allocator(){
  if(alpha!= NULL)
  {
  delete[] alpha;
...
void Allocator::loadRooms(const string& file)
{
    ...
    //first you need to initialize the roomCount first
    //otherwise you dont have the variable initialized first
    roomCount = fileio::getNumRooms();
    rooms = new Room[roomCount];
    totalCapacity = 0;
    int i = 0;
    while (fileio::areMoreRooms()) {
      //increment later so you wont save anything in the index=0 of your array
        //i++;
