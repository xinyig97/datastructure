...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode* KDTreeHelper(vector<Point<Dim>> newPoints, int dim, int start, int end);
    void quickSort(vector<Point<Dim>>& newPoints, int first, int last,
                                int dim);
    KDTreeNode* findNearest_Helper(const Point<Dim>& query, KDTreeNode* curr, KDTreeNode* best, int split) const;

    double dist(Point<Dim> point1, Point<Dim> point2) const;
