                            ...
                            queue<TreeNode*>& mergeQueue)
{
    if(singleQueue.empty()&&mergeQueue.empty()){
      ...
      TreeNode * temp_1 = mergeQueue.front();
      mergeQueue.pop();
      return temp_1;
    }
    else if(!singleQueue.empty()&&mergeQueue.empty()){
      TreeNode * temp_2 = singleQueue.front();
      singleQueue.pop();
      return temp_2;
    }
    else{
    TreeNode* temp_1 = singleQueue.front();
    TreeNode* temp_2 = mergeQueue.front();
    if((temp_2->freq).getFrequency() < (temp_1->freq).getFrequency()){
      mergeQueue.pop();
      return temp_2;
    }
    else{
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(!singleQueue.empty()){
      TreeNode * temp_1 = removeSmallest(singleQueue,mergeQueue);
      TreeNode * temp_2 = removeSmallest(singleQueue,mergeQueue);
      TreeNode * temp_internal = new TreeNode((temp_1->freq).getFrequency()+(temp_2->freq).getFrequency());
      temp_internal->left = temp_1;
      temp_internal->right = temp_2;
      mergeQueue.push(temp_internal);
    }
...
void HuffmanTree::decode(stringstream& ss, BinaryFileReader& bfile)
{
    ...
    while (bfile.hasBits()) {
      if(current->left == NULL && current->right == NULL){
        char temp_r = (current->freq).getCharacter();
        ss<< temp_r;
        current = root_;
      }
      else{
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    if(bfile.hasBits()){
      if(bfile.getNextBit() == true){
        ...
        TreeNode * temp_tree = new TreeNode(temp_freq);
        return temp_tree;
      }
      else{
        TreeNode * temp_internal = new TreeNode(0);
        temp_internal->left = readTree(bfile);
        temp_internal->right = readTree(bfile);
